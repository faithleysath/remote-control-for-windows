...
当前 workspace 版本是 `0.1.9`，协议版本是 `v6`。项目已经从 v1 的从零实现阶段进入长期维护和迭代阶段；主链路已经在真实 Windows VM 中完成主要闭环验证，后续工作应在保持当前安全模型的前提下继续强化打包、自动化验证和操作体验。
...
- `rcw-host-gui`：Tauri v2 GUI host 工程，提供概览、会话、Exec 任务、审计和设置页 MVP。
