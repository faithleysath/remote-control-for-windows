...
workspace 当前版本为 `0.1.11`。在采用更完整的兼容性策略前：
...
也可以在 GitHub Actions 页面手动运行 `Release` workflow，并填写 tag，例如 `v0.1.11`。手动运行时 workflow 会以当前 commit 为目标创建 release tag。
...
5. 按 `docs/testing.md` 的 `release validation` 子集运行或刷新 Windows 交互桌面 `pytest` smoke，并保留本轮验证结论。
