...
所有 hello/open 消息都带 `protocol_version`。当前实现的协议版本是 `4`，server 对 `host.hello` 和 `control.open` 都要求版本精确匹配 `PROTOCOL_VERSION`；不匹配时返回 `error` / `internal_error`。当前不做协议范围兼容协商；后续协议扩展必须保持既有字段含义不变，或有意识地提升协议版本。
