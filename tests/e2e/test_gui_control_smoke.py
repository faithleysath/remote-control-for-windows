...
@pytest.mark.smoke
def test_gui_control_smoke(
    test_config: TestConfig,
    smoke_output_dir,
    gui_host_runtime: GuiAutomationConnectionInfo,
    ...
    connect = run_rcwctl_json(
        ...
        [
            "connect",
            "--id",
            gui_host_runtime.machine_id,
            "--host-id",
            gui_host_runtime.host_id,
            "--totp",
            gui_host_runtime.totp,
        ],
    ...
    screenshot = run_rcwctl_json(
        test_config,
        rcwctl_env,
        ["screenshot", "--output", str(smoke_output_dir / "gui-host.png")],
    ...
    assert (smoke_output_dir / "gui-host.png").is_file()
    ...
    summary = {
        "machine_id": gui_host_runtime.machine_id,
        "host_id": gui_host_runtime.host_id,
        "listener_status": gui_host_runtime.listener_status,
        "connect_request_id": connect["request_id"],
        "status_request_id": status["request_id"],
        "windows_count": len(windows["windows"]),
        "screenshot_path": str(smoke_output_dir / "gui-host.png"),
